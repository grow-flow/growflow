...
const CreatePlantDialog: React.FC<CreatePlantDialogProps> = ({
  ...
  const generatePlantName = (strain: Strain | null, strainName: string) => {
    const abbreviation =
      strain?.abbreviation || strainName.substring(0, 3).toUpperCase();
    const existingPlants = plants.filter((p) =>
      p.name.startsWith(abbreviation)
    );
  ...
  const handleNewStrainInputChange = (_: any, newInputValue: string) => {
    ...
    if (
      newInputValue &&
      !strains.find((s) => s.name.toLowerCase() === newInputValue.toLowerCase())
    ) {
      setFormData({ ...formData, strain_id: null });
    }
  };
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      start_method: e.target.value as StartMethod,
                    })
                  }
                >
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, medium: e.target.value as any })
                  }
                >
          ...
          {newStrainName &&
            !strains.find(
              (s) => s.name.toLowerCase() === newStrainName.toLowerCase()
            ) && (
